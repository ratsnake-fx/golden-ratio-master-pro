...
with st.sidebar:
    ...
    if config.get("type") == "futures":
        ...
        # Lot Step Option for Forex/CFD
        st.divider()
        st.markdown("**Lot-Einstellungen**")
        lot_step = st.select_slider(
            "Min. Lot-Grösse",
            options=[0.01, 0.1, 1.0],
            value=0.01,
            help="Minimale handelbare Lot-Grösse deines Brokers"
        )
        st.caption(f"→ Lots werden auf {lot_step} gerundet")
