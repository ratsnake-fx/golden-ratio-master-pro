...
def calculate_optimal_lot_step(account: float, target_risk_pct: float, entry: float, sl_max: float, symbol: str) -> dict:
    """
    Calculate the optimal lot step that keeps actual risk <= target risk.
    Returns dict with lot_step, actual_risk, actual_risk_pct, and warning if needed.
    """
    config = get_symbol_config(symbol)
    target_risk_usd = account * (target_risk_pct / 100)
    sl_dist_max = abs(entry - sl_max)
    
    is_futures = config.get("type") == "futures"
    
    if is_futures:
        # Futures: minimum is 1 contract
        point_value = config["point_value"]
        min_risk_per_contract = sl_dist_max * point_value
        
        # Calculate how many contracts we can afford with 5-split
        # Smallest position is P5 with 2/31 weight and 14.6% decay
        p5_target_risk = (2 / 31) * target_risk_usd
        p5_sl_dist = sl_dist_max * 0.146
        p5_min_risk = p5_sl_dist * point_value  # Risk for 1 contract
        
        if p5_min_risk > p5_target_risk:
            # Can't meet target risk with 1 contract
            actual_total_risk = sum([
                sl_dist_max * decay * point_value 
                for decay in GOLDEN_DECAY
            ])
            return {
                "lot_step": 1,
                "actual_risk_usd": actual_total_risk,
                "actual_risk_pct": (actual_total_risk / account) * 100,
                "target_risk_usd": target_risk_usd,
                "target_risk_pct": target_risk_pct,
                "warning": True,
                "warning_msg": f"Minimum 1 Kontrakt pro Position. Ist-Risiko: ${actual_total_risk:.2f} ({(actual_total_risk/account)*100:.2f}%)"
            }
        ...
        # Forex/CFD: Find optimal lot step
        pip_mult = config["pip_mult"]
        pip_value = config["pip_value"]
        pip_value_per_lot = pip_value  # Pip value per 1 standard lot per pip
        
        available_lot_steps = [0.01, 0.1, 1.0]
        
        for lot_step in available_lot_steps:
            # Calculate actual risk with this lot step
            total_actual_risk = 0
            
            for i in range(5):
                target_pos_risk = (FIBONACCI_WEIGHTS[i] / WEIGHT_SUM) * target_risk_usd
                pos_sl_dist = sl_dist_max * GOLDEN_DECAY[i]
                sl_pips = pos_sl_dist * pip_mult
                
                # Theoretical lots
                theoretical_lots = target_pos_risk / (sl_pips * pip_value_per_lot) if sl_pips > 0 else 0
                
                # Round to lot_step
                rounded_lots = max(round(theoretical_lots / lot_step) * lot_step, lot_step)
                
                # Actual risk
                actual_pos_risk = rounded_lots * sl_pips * pip_value_per_lot
                total_actual_risk += actual_pos_risk
            
            actual_risk_pct = (total_actual_risk / account) * 100
            
            # If actual risk is <= target (with small tolerance), use this lot step
            if total_actual_risk <= target_risk_usd * 1.05:  # 5% tolerance
                return {
                    "lot_step": lot_step,
                    "actual_risk_usd": total_actual_risk,
        ...
        # If we get here, even 0.01 lots exceeds target risk
        # Recalculate with 0.01 lots
        lot_step = 0.01
        total_actual_risk = 0
        ...
        for i in range(5):
            pos_sl_dist = sl_dist_max * GOLDEN_DECAY[i]
            sl_pips = pos_sl_dist * pip_mult
            actual_pos_risk = lot_step * sl_pips * pip_value_per_lot
            total_actual_risk += actual_pos_risk
        ...
        return {
            ...
            "target_risk_usd": target_risk_usd,
            "target_risk_pct": target_risk_pct,
            "warning": total_actual_risk > target_risk_usd,
            "warning_msg": f"⚠️ Minimum 0.01 Lot. Ist-Risiko (${total_actual_risk:.2f} / {actual_risk_pct:.2f}%) > Soll-Risiko (${target_risk_usd:.2f} / {target_risk_pct:.2f}%)" if total_actual_risk > target_risk_usd else ""
        }


def calculate_positions(account: float, risk_pct: float, entry: float, sl_max: float, tp: float, symbol: str, lot_step: float = 0.01) -> pd.DataFrame:
    """
    Calculate all 5 positions with Golden Ratio decay.
    
    WICHTIG: Lots werden auf handelbare Grössen gerundet (Standard: 0.01).
    Das tatsächliche Risiko wird dann aus den gerundeten Lots berechnet!
    Wenn das Ziel-Risiko zu klein ist, wird minimum 1x lot_step pro Position verwendet.
    """
    config = get_symbol_config(symbol)
    target_total_risk = account * (risk_pct / 100)
    ...
    if is_futures:
        point_value = config["point_value"]
        pip_mult = 1  # Futures use points directly
        pip_value_per_lot = point_value
        unit_label = "Kontrakte"
        min_size = 1  # Minimum 1 contract for futures
    else:
        pip_mult = config["pip_mult"]
        pip_value = config["pip_value"]
        pip_value_per_lot = pip_value  # Pip value per 1 standard lot per pip
        unit_label = "Lots"
        min_size = lot_step  # Minimum lot size (e.g., 0.01)
    
    data = []
    for i in range(5):
        # Target risk allocation based on Fibonacci weights
        target_pos_risk = (FIBONACCI_WEIGHTS[i] / WEIGHT_SUM) * target_total_risk
        ...
        if is_futures:
            sl_pips = pos_sl_dist  # Points for futures
            tp_pips = tp_dist
        else:
            sl_pips = pos_sl_dist * pip_mult  # Pips for forex
            tp_pips = tp_dist * pip_mult
        
        # Calculate theoretical lot size
        if is_futures:
            theoretical_size = target_pos_risk / (pos_sl_dist * point_value) if pos_sl_dist > 0 else 0
            # Round to whole contracts (minimum 1)
            rounded_size = max(round(theoretical_size), 1)  # Always minimum 1 contract
        else:
            theoretical_size = target_pos_risk / (sl_pips * pip_value_per_lot) if sl_pips > 0 else 0
            # Round to lot_step - use minimum lot_step if theoretical is too small
            rounded_size = round(theoretical_size / lot_step) * lot_step
            if rounded_size < lot_step:
                rounded_size = lot_step  # Minimum 1x lot_step
        
        # Calculate ACTUAL risk from rounded lots
        if is_futures:
            actual_risk = rounded_size * pos_sl_dist * point_value
            actual_profit = rounded_size * tp_dist * point_value
        else:
            actual_risk = rounded_size * sl_pips * pip_value_per_lot
            actual_profit = rounded_size * tp_pips * pip_value_per_lot
        ...
        data.append({
            ...
            "SL_Pips": sl_pips,
            "TP_Pips": tp_pips,
            "Target_Risk_USD": target_pos_risk,
            "Theoretical_Lots": theoretical_size,
            "Lots": rounded_size,
...
def calculate_single_entry(account: float, risk_pct: float, entry: float, sl: float, tp: float, symbol: str, lot_step: float = 0.01) -> dict:
    """Calculate single entry position for comparison with lot rounding."""
    config = get_symbol_config(symbol)
    target_risk = account * (risk_pct / 100)
    ...
    if is_futures:
        point_value = config["point_value"]
        sl_pips = sl_dist  # Points
        tp_pips = tp_dist
        
        # Calculate theoretical size and round to whole contracts
        theoretical_size = target_risk / (sl_dist * point_value) if sl_dist > 0 else 0
        rounded_size = max(round(theoretical_size), 0)
        
        # Actual risk from rounded size
        actual_risk = rounded_size * sl_dist * point_value
        actual_profit = rounded_size * tp_dist * point_value
        unit_label = "Kontrakte"
    else:
        pip_mult = config["pip_mult"]
        ...
        # Calculate theoretical size and round to lot_step
        theoretical_size = target_risk / (sl_pips * pip_value_per_lot) if sl_pips > 0 else 0
        rounded_size = max(round(theoretical_size / lot_step) * lot_step, 0)
...
with st.sidebar:
    ...
    risk_pct = st.number_input("Soll-Risiko (%)", value=0.25, min_value=0.01, max_value=10.00, step=0.01, format="%.2f")
    ...
    if config.get("type") == "futures":
        ...
        if futures_mode == "Kleinst-System (1 Kontrakt)":
            ...
            multiplier = 1
        lot_step = 1  # Futures always use whole contracts
    else:
    ...
    # Calculate optimal lot step for Forex/CFD (after we have entry/sl values)
    if config.get("type") != "futures":
        st.divider()
        st.markdown("**Lot-Berechnung**")
        
        # Calculate optimal lot step
        lot_calc = calculate_optimal_lot_step(account, risk_pct, entry, sl_max, symbol)
        lot_step = lot_calc["lot_step"]
        
        # Display results
        st.markdown(f"""
        <div style="background:#1e293b; padding:0.75rem; border-radius:8px; margin:0.5rem 0;">
            <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem;">
                <span style="color:#94a3b8;">Min. Lot-Grösse:</span>
                <span style="color:#f59e0b; font-weight:bold;">{lot_step}</span>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem;">
                <span style="color:#94a3b8;">Soll-Risiko:</span>
                <span style="color:#64748b;">${lot_calc['target_risk_usd']:.2f} ({lot_calc['target_risk_pct']:.2f}%)</span>
            </div>
            <div style="display:flex; justify-content:space-between;">
                <span style="color:#94a3b8;">Ist-Risiko:</span>
                <span style="color:{'#ef4444' if lot_calc['warning'] else '#10b981'}; font-weight:bold;">${lot_calc['actual_risk_usd']:.2f} ({lot_calc['actual_risk_pct']:.2f}%)</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        if lot_calc["warning"]:
            st.warning(lot_calc["warning_msg"])
...
if is_futures_min_mode:
    ...
    single = {
        "size": multiplier,
        "theoretical_size": multiplier,
        "target_risk": multiplier * abs(entry - sl_max) * config["point_value"],
        "risk": multiplier * abs(entry - sl_max) * config["point_value"],
        "risk_pct": (multiplier * abs(entry - sl_max) * config["point_value"] / account) * 100,
        "profit": multiplier * abs(entry - tp) * config["point_value"],
        "rr": abs(entry - tp) / abs(entry - sl_max) if abs(entry - sl_max) > 0 else 0,
        "sl_pips": abs(entry - sl_max),
        "tp_pips": abs(entry - tp),
        "unit": "Kontrakte"
    }
    futures_validation = {"valid": True}  # Always valid in min mode
else:
    df_positions = calculate_positions(account, risk_pct, entry, sl_max, tp, symbol, lot_step)
    single = calculate_single_entry(account, risk_pct, entry, sl_max, tp, symbol, lot_step)
...
with tab1:
    ...
    with col_a:
        ...
        | **Ziel-Risiko** | ${single['target_risk']:.2f} ({risk_pct:.2f}%) |
        | **Echtes Risiko** | ${single['risk']:.2f} ({single['risk_pct']:.2f}%) |
        | **SL {points_label}** | {single['sl_pips']:.1f} |
        | **RR** | {single['rr']:.2f}x |
    ...
    with col_b:
        ...
        total_risk_pct = df_positions['Risk_Pct'].sum() if 'Risk_Pct' in df_positions.columns else (total_risk / account) * 100
        target_total_risk = df_positions['Target_Risk_USD'].sum() if 'Target_Risk_USD' in df_positions.columns else account * (risk_pct / 100)
        
        st.markdown(f"""
        | Metrik | Wert |
        |--------|------|
        | **Total {unit_label}** | {total_size:.2f} |
        | **Ziel-Risiko** | ${target_total_risk:.2f} ({risk_pct:.2f}%) |
        | **Echtes Risiko** | ${total_risk:.2f} ({total_risk_pct:.2f}%) |
    ...
    with col_p:
        ...
        for _, row in df_positions.iterrows():
            ...
            if 'Contracts' in row and 'SL_Ticks' in row:
                ...
                # Standard Forex/CFD Layout - Enhanced with all details
                unit = row.get('Unit', 'Lots')
                weight = row.get('Weight', row['Decay'])
                size_val = row.get('Lots', row.get('Contracts', 0))
                risk_pct_pos = row.get('Risk_Pct', 0)
                sl_pips = row.get('SL_Pips', 0)
                ...
                <div class="position-row">
                    ...
                    <div style="display:grid; grid-template-columns: repeat(5, 1fr); gap:0.5rem; margin-top:0.75rem; font-size:0.8rem;">
                        <div style="background:#1e293b; padding:0.5rem; border-radius:6px; text-align:center;">
                            <div style="color:#64748b; font-size:0.7rem;">{unit.upper()}</div>
                            <div style="color:#f59e0b; font-weight:600;">{size_val:.2f}</div>
                        </div>
                        <div style="background:#1e293b; padding:0.5rem; border-radius:6px; text-align:center;">
                            <div style="color:#64748b; font-size:0.7rem;">SL PREIS</div>
                            <div style="color:#ef4444; font-weight:600;">{row['SL_Price']:.{digits}f}</div>
                        ...
                        <div style="background:#1e293b; padding:0.5rem; border-radius:6px; text-align:center;">
                            <div style="color:#64748b; font-size:0.7rem;">SL PIPS</div>
                            <div style="color:#94a3b8; font-weight:600;">{sl_pips:.1f}</div>
                        </div>
                        <div style="background:#1e293b; padding:0.5rem; border-radius:6px; text-align:center;">
                            <div style="color:#64748b; font-size:0.7rem;">RISK %</div>
                            <div style="color:#ef4444; font-weight:600;">{risk_pct_pos:.3f}%</div>
                        </div>
                        <div style="background:#1e293b; padding:0.5rem; border-radius:6px; text-align:center;">
                            <div style="color:#64748b; font-size:0.7rem;">RR</div>
                            <div style="color:#06b6d4; font-weight:600;">{row['RR']:.1f}x</div>
